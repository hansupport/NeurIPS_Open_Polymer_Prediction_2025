...
# output
NUM_OF_OUTPUT_DIR = 1
OUTPUT_DIR = Path("Output") / f"Output{NUM_OF_OUTPUT_DIR}"
...
def augment_smiles_dataset(smiles_list, labels, num_augments=AUG_NUM):
    ...
    for smi, lab in zip(smiles_list, labels):
        ...
        if mol is None:
            continue
        aug_smi.append(smi); aug_lbl.append(lab)
...
def smiles_to_combined_fingerprints_with_descriptors(smiles_list, radius=2, n_bits=128):
    ...
    for i, smi in enumerate(smiles_list):
        ...
        if mol:
            ...
            descs.append(dv); valid.append(smi)
        else:
            fps.append(np.zeros(n_bits+167))
            descs.append({}); invalid_idx.append(i); valid.append(None)
...
def visualize_overall_results(y_true, y_pred, train_histories, val_histories, target, model_name, step_size=1,
    ...
    try:
        ...
        if model_name == 'ET':
            ...
            ax_train_curve.plot(x_axis, mean_train_mae, 'o-', color='tab:blue', label='Average Train MAE', markersize=4)
            ax_train_curve.fill_between(x_axis, mean_train_mae - std_train_mae, mean_train_mae + std_train_mae,
                                        color='tab:blue', alpha=0.15)
            ax_train_curve.set_title(f'Overall Train Performance (Avg over {len(train_histories)} Folds)', fontsize=14)
        ...
        # 2. 그 외 트리 모델 (LGBM, XGB, CAT): STEP 간격으로 점을 찍어 통합 표시
            ...
            ax_lr_curve.scatter(plot_x_axis[best_val_idx], plot_val_mae[best_val_idx], color='darkorange', s=100,
                                zorder=5, edgecolor='black', label=f'Best Val MAE: {plot_val_mae[best_val_idx]:.5f}')
        ...
        # 3. FTT 모델: 모든 Epoch을 표시
            ...
            ax_lr_curve.plot(x_axis, mean_train_mae, 'o-', color='tab:blue', label='Average Train MAE', markersize=4)
            ...
            ax_lr_curve.fill_between(x_axis, mean_train_mae - std_train_mae, mean_train_mae + std_train_mae,
                                     color='tab:blue', alpha=0.15)
            ax_lr_curve.fill_between(x_axis, mean_val_mae - std_val_mae, mean_val_mae + std_val_mae, color='tab:orange',
                                     alpha=0.15)

            best_train_idx = np.argmin(mean_train_mae);
            best_val_idx = np.argmin(mean_val_mae)
            ax_lr_curve.scatter(x_axis[best_train_idx], mean_train_mae[best_train_idx], color='blue', s=100, zorder=5,
                                edgecolor='black', label=f'Best Train MAE: {mean_train_mae[best_train_idx]:.5f}')
            ax_lr_curve.scatter(x_axis[best_val_idx], mean_val_mae[best_val_idx], color='darkorange', s=100, zorder=5,
            ...
            for fold, (train_idx, val_idx) in enumerate(
                ...
                if model_name == 'FTT':
                    ...
                    if model_name == 'ET':
                        ...
                        diag_pipeline_steps = []
                        if DO_VARIANCE_THRESHOLD:
                            diag_pipeline_steps.append(('selector', VarianceThreshold(threshold=VARIANCE_THRESHOLD)))
                        if DO_StandardScaler:
                    ...
                    try:
                        ...
                        if hasattr(model_step, 'feature_importances_'):
                            ...
                            if 'selector' in pipeline.named_steps:
                                ...
                                final_feature_names = [name for name, keep in zip(current_feature_names, selector_mask)
                                                       if keep]
